...
public class DefaultResponseEntityBuilder implements ResponseEntityBuilder {
     ...
     * {@inheritDoc}
     */
    public <T> ResponseEntity<ResponseObject<T>> build(T data, HttpStatus responseState) {
        final ResponseObject<T> ro = ResponseObject.<T>builder().data(data).build();
        return new ResponseEntity<>(ro, responseState);
    }
}
