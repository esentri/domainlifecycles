...
package io.domainlifecycles.mirror.exception;

import static java.util.Objects.requireNonNull;

/**
 * RuntimeException that reports errors on initializing or accessing the mirror.
 * Should should only be used for cases where the code is bad, e.g. because a case is not covered that is
...
public class MirrorException extends RuntimeException {
    ...
    public static MirrorException fail(final String detail, final Throwable cause) {
        return new MirrorException(requireNonNull(detail), requireNonNull(cause));
    }
