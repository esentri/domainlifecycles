...
import io.domainlifecycles.jackson2.databind.context.DomainObjectMappingContextHolder;
    ...
    private void readAndSetValueObjectField(DomainObjectMappingContext mappingContext,
        ...
        if (jsonNode != null) {
            ...
            try {
                ...
                if (customizer != null) {
                    mappingAction = customizer.beforeFieldRead(mappingContext, jsonNode, fieldName, fieldType, objectCodec);
                }
                if (MappingAction.CONTINUE_WITH_DEFAULT_ACTION.equals(mappingAction)) {
                    Object value;
                    if (ValueObject.class.isAssignableFrom(fieldType)) {
                        var p = jsonNode.traverse(objectCodec);
                        value = deserializationContext.readValue(p, fieldType);
                    } else {
                        value = jsonNode.traverse(objectCodec).readValueAs(fieldType);
                    }
