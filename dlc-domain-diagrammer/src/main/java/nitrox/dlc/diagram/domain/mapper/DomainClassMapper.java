...
import nitrox.dlc.diagram.nomnoml.NomnomlClass;
import nitrox.dlc.diagram.nomnoml.NomnomlField;
import nitrox.dlc.diagram.nomnoml.NomnomlMethod;
import nitrox.dlc.diagram.nomnoml.NomnomlParameter;
import nitrox.dlc.diagram.nomnoml.NomnomlStereotype;
import nitrox.dlc.diagram.nomnoml.NomnomlType;
...
public class DomainClassMapper {
                ...
                if(List.of(
                    ...
                    if(DomainType.VALUE_OBJECT.equals(domainTypeMirror.getDomainType())){
                        ...
                        if(!voMirror.isSingledValued()){
                            aggregateClasses.add(mapToNomnomlClass(domainTypeMirror, domainDiagramConfig.isShowFields(), domainDiagramConfig.isShowMethods()));
                        }
                    }else {
                        aggregateClasses.add(mapToNomnomlClass(domainTypeMirror, domainDiagramConfig.isShowFields(), domainDiagramConfig.isShowMethods()));
                    }
    ...
    private NomnomlClass mapToNomnomlClass(DomainTypeMirror domainTypeMirror, boolean showFields, boolean showMethods){
        ...
        var nomnomlClassBuilder = NomnomlClass
            ...
            .stereotypes(List.of(NomnomlStereotype.builder().name(DomainMapperUtils.stereotype(domainTypeMirror, domainDiagramConfig)).build()))
            .name(className)
        ...
        if(showFields){
            ...
            if(domainTypeMirror.getDomainType().equals(DomainType.ENTITY)||domainTypeMirror.getDomainType().equals(DomainType.AGGREGATE_ROOT)){
    ...
    private NomnomlType mapToNomnomlType(AssertedContainableTypeMirror assertedContainableTypeMirror){
        ...
        if(assertedContainableTypeMirror.getResolvedGenericType() == null){
            ...
            containerTypeName = assertedContainableTypeMirror.getContainerTypeName().map(cn -> DomainMapperUtils.mapTypeName(cn, domainDiagramConfig));
        }else{
            var genericType = assertedContainableTypeMirror.getResolvedGenericType();
            if(assertedContainableTypeMirror.hasContainer()){
                typeName = DomainMapperUtils.mapTypeName(genericType.genericTypes().get(0).typeName(), domainDiagramConfig);
                ...
                if(genericType.isArray()){
                    ...
                    containerTypeName = Optional.of(DomainMapperUtils.mapTypeName(genericType.typeName(), domainDiagramConfig));
